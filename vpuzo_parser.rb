...
db.transaction do |db|
  db.execute("CREATE TABLE IF NOT EXISTS authors (id INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL, name text, rating float,
                link text, created_at datetime default current_timestamp, updated_at datetime default current_timestamp)")
  db.execute("CREATE TABLE IF NOT EXISTS recipes (id INTEGER PRIMARY KEY NOT NULL, name text, author text, image text,
  ...
  uniq_authors.each do |author|
    db.execute("INSERT INTO authors (name, rating, link)
                VALUES (?, ?, ?)",
               author[:author], author[:author_rating], author[:author_link])
  end
  recipes.each do |recipe|
    db.execute("INSERT INTO recipes (name, image, author, cook_time, geography, main_ingredient, type, link)
               VALUES (?, ?, ?, ?, ?, ?, ?, ?)",
